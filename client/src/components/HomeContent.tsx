...
function interpretFrequencies(
  dominantFreqs: Array<{ freq: number; magnitude: number }>
): string[] {
...
function calculateFFT(signal: number[]): {
  frequencies: number[];
  magnitudes: number[];
} {
...
function getMaxProbability(probabilities: Record<string, number> | undefined): number {
...
function HomeContent() {
  ...
  const [dominantFreqs, setDominantFreqs] = useState<{
    [key: string]: Array<{ freq: number; magnitude: number }>;
  }>({});
  ...
  const fetchMonitoringStatus = async () => {
    try {
      ...
      if (data.status === "running" && data.prediction) {
        ...
        // Check for alert conditions and create notification
        const probabilities = data.prediction.details.class_probabilities;
        if (probabilities["desiquilibre"] >= 0.4 || probabilities["cassure"] >= 0.4) {
          const severity = probabilities["cassure"] >= 0.4 ? "ÉLEVÉ" : "MOYEN";
          const eventData: Event = {
            _id: Math.random().toString(36).substr(2, 9),
            fileName: "monitoring_alert",
            time: new Date().toISOString(),
            type: "MONITORING",
            severity,
            description: [
              { key: "state", value: data.prediction.state },
              { key: "confidence", value: `${(Math.max(...Object.values(probabilities)) * 100).toFixed(2)}%` },
              { key: "alert", value: probabilities["cassure"] >= 0.4 ? "Défaut Critique Détecté" : "Attention - Déséquilibre Détecté" }
            ],
            predictionDetails: {
              predictedClass: data.prediction.state,
          ...
          // Send notification
          console.log("Creating monitoring alert notification:", eventData);
      ...
      if (response.ok) {
        ...
        if (result.signals) {
          ...
          Object.entries(result.signals).forEach(([signalName, data]) => {
            ...
            const peaks = findDominantFrequencies(
              fftData.frequencies,
              fftData.magnitudes
            );
          ...
          try {
            ...
            const maxProb = getMaxProbability(result.class_probabilities);
            
            if (result.prediction === "cassure" && maxProb > 0.4) {
              severity = "ÉLEVÉ";
            } else if (result.prediction === "desiquilibre" && maxProb > 0.4) {
              severity = "MOYEN";
            } else if (maxProb > 0.4) {
              severity = "NORMAL";
            }
            ...
            const eventData: Event = {
              ...
              description: [
                ...
                { key: "confidence", value: `${(maxProb * 100).toFixed(2)}%` },
                ...Object.entries(newDominantFreqs).map(
                  ([signalName, freqs]) => ({
                    key: `signal_${signalName}_stats`,
                    value: freqs
                      .map(
                        (f) =>
                          `${f.freq.toFixed(2)}Hz (${f.magnitude.toFixed(2)})`
                      )
                      .join(", "),
                  })
                ),
            ...
            if (eventResponse.ok) {
              ...
              if (responseData.success && responseData.data) {
                // Trigger notification for any severity level except FAIBLE
                console.log("Checking severity for notification:", severity);
                if (severity !== "FAIBLE") {
                  console.log("Triggering notification for event:", responseData.data);
                  addNotification(responseData.data);
                }
              }
            }
          } catch (eventError) {
    ...
    <div className="space-y-6 p-6">
      ...
      {!uploadedData && !isMonitoring && (
        <div className="text-center p-8 bg-gray-50 rounded-lg">
          <p className="text-xl text-gray-600">
            Téléchargez votre fichier de signal pour commencer l'analyse
          </p>
        </div>
      )}
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <div className="text-center">
                  ...
                  <div
                    className={`text-3xl font-bold mb-2 ${
                      monitoringData.prediction.details.class_probabilities[
                        "sain"
                      ] >= 0.4
                        ? "text-green-600"
                        : monitoringData.prediction.details.class_probabilities[
                  ...
                  <div className="text-sm text-gray-600 mb-4">
                    Confiance:{" "}
                    {(getMaxProbability(monitoringData.prediction.details.class_probabilities) * 100).toFixed(1)}%
                  </div>
                  ...
                  <div className="mt-4 text-left p-4 bg-gray-50 rounded">
                    {monitoringData.prediction.details.class_probabilities[
                      ...
                      <div className="text-green-700">
                        <p className="font-medium">Aucun symptôme détecté.</p>
                        <p>
                          Les performances sont stables, sans surchauffe ni
                          anomalies vibratoires.
                        </p>
                      </div>
                    ) : monitoringData.prediction.details.class_probabilities[
                        "desiquilibre"
                      ...
                      <div className="text-yellow-700">
                        <p className="font-medium">
                          Présence de vibrations anormales et de perturbations
                          électriques.
                        </p>
                        <p>
                          Actions recommandées : réalignement de la machine et
                          vérification de la protection électrique.
                        </p>
                      </div>
                    ) : monitoringData.prediction.details.class_probabilities[
                        "cassure"
                      ...
                      <div className="text-red-700">
                        <p className="font-medium">
                          Bruits anormaux et augmentation du courant électrique.
                        </p>
                        <p>
                          Action corrective : remplacement des composants
                          internes défectueux.
                        </p>
                      </div>
                    ) : (
                      <div className="text-gray-700">
                        <p>État indéterminé</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="h-64">
                  <Doughnut
                    data={{
                      labels: [
                        "Normal (Sain)",
                        "Attention (Déséquilibre)",
                        "Critique (Cassure)",
                      ],
                      datasets: [
                        {
                          data: [
                            (monitoringData.prediction.details
                              .class_probabilities["sain"] || 0) * 100,
                            (monitoringData.prediction.details
                              .class_probabilities["desiquilibre"] || 0) * 100,
                            (monitoringData.prediction.details
                              .class_probabilities["cassure"] || 0) * 100,
                          ],
                          backgroundColor: [
              ...
              <div className="col-span-2 p-6 border rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(
                    monitoringData.prediction.details.validation_patterns
                  ).map(([pattern, isPresent]) => (
                    <div key={pattern} className="flex items-center space-x-2">
                      ...
                      ></div>
                      <span>
                        {pattern}: {isPresent ? "Présent" : "Absent"}
                      </span>
                    </div>
                  ))}
                </div>
      ...
      {!isMonitoring && uploadedData && (
        <div className="grid grid-cols-1 gap-6">
          {/* Prediction Results - Moved to top */}
          {prediction && (
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold mb-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    ...
                    <p className="text-sm text-gray-600">
                      ...
                      {prediction.class_probabilities
                        ? `${(getMaxProbability(prediction.class_probabilities) * 100).toFixed(2)}%`
                        : "N/A"}
                    </p>
                    {prediction.class_probabilities && (
                      <div className="mt-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    <div className="grid grid-cols-3 gap-1 text-center text-sm">
                      ...
                      {["cassure", "sain", "desiquilibre"].map((label) => (
                        <div key={label} className="font-bold">
                          {label}
                        </div>
                      ))}
                      {prediction.metrics.confusionMatrix.map((row, i) => (
                        <>
                          <div className="font-bold">
                            {["cassure", "sain", "desiquilibre"][i]}
                          </div>
                          {row.map((value, j) => (
                            <div
                              key={j}
                              className={`p-2 ${
                                i === j ? "bg-blue-100" : "bg-gray-100"
                              }`}
                            >
                              {(value * 100).toFixed(2)}%
                            </div>
                ...
                <div className="col-span-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Bar
                        data={{
                          labels: prediction.metrics.classMetrics.map(
                            (m) => m.class
                          ),
                          datasets: [
                            {
                              label: "Précision",
                              data: prediction.metrics.classMetrics.map(
                                (m) => m.precision * 100
                              ),
                      ...
                      <div className="mt-4">
                        ...
                        {prediction.metrics.classMetrics.map((metric) => (
                          <div key={metric.class} className="mt-2">
                            ...
                            <p className="text-sm">
                              VP:{" "}
                              {(metric.precision * metric.recall * 100).toFixed(
                                2
                              )}
                              %
                            </p>
                            <p className="text-sm">
                              FP: {((1 - metric.precision) * 100).toFixed(2)}%
                            </p>
                            <p className="text-sm">
                              FN: {((1 - metric.recall) * 100).toFixed(2)}%
                            </p>
                            <p className="text-sm">
                              VN:{" "}
                              {(
                ...
                <div className="col-span-2">
                  ...
                  <div className="h-64">
                    <Line
                      data={{
                        labels: prediction.metrics.rocCurve.map((point) =>
                          point.x.toFixed(2)
                        ),
                        datasets: [
                          {
                            label: "Courbe ROC",
                            data: prediction.metrics.rocCurve.map(
                              (point) => point.y
                            ),
          ...
          {/* Signal Plots - Moved below prediction results */}
          <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div key={signalName} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <Line
                          ...
                          options={{
                            ...
                            plugins: {
                              title: {
                                display: true,
                                text: "Signal dans le domaine temporel",
                              },
                              zoom: {
                                limits: {
                                  y: { min: -2, max: 2 },
                      ...
                      <div className="h-64">
                        <Line
                          data={{
                            labels: fftData.frequencies.map((f) =>
                              f.toFixed(0)
                            ),
                            datasets: [
                              {
                      ...
                      <div className="col-span-2 mt-4">
                        <h4 className="font-medium mb-2">
                          Fréquences dominantes et interprétation:
                        </h4>
                        <div className="grid grid-cols-2 gap-4">
                          <ul className="list-disc pl-5">
                            {peaks.map((peak, i) => (
                              <li key={i}>
                                {peak.freq.toFixed(2)} Hz (Amplitude:{" "}
                                {peak.magnitude.toFixed(2)})
                              </li>
                            ))}
                          </ul>
